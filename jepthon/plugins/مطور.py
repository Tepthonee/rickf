...
JEP = gvarstatus("sudoenable") or "true"
...
@jepiq.ar_cmd(
    pattern="جعل المطور (تفعيل|تعطيل)$",
    command=("جعل المطور", plugin_category),
    info={
...
async def chat_blacklist(event):
    ...
    if input_str == "تفعيل":
        ...
        if len(sudousers) != 0:
            text += (
                "**✾╎يتم الان اعـادة تشغيـل بـوت جيبثون انتظـر 2-1 دقيقـه ▬▭ ...**"
            )
...
async def add_sudo_user(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت جيبثون انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن  :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت جيبثون انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
