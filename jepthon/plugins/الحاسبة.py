...
async def icalc(e):
    if e.client._bot:
        return await e.reply(
            "**الحـاسبة العـلمية لسـورس تيبثونn @Tepthone**", buttons=lst
        )
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**الحـاسبة العـلمية لسـورس تيبثون\n @Tepthone**", buttons=lst
        )
...
async def _(e):  # sourcery no-metrics
    ...
    if x == "AC":
        if CALC.get(user):
            CALC.pop(user)
        await e.edit(
            "**الحـاسبة العـلمية لسـورس تيبثون\n @Tepthone**",
...
async def _(e):
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**الحـاسبة العـلمية لسـورس تيبثون\n @Tepthone**", buttons=lst)
